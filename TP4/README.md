...
## TP4 : Traitement d'images en Python 
...
Si vous n'êtes pas familier avec `numpy`, commencer par lire cette page :

https://numpy.org/devdocs/user/absolute_beginners.html

Puis, lire attentivement ces pages pour l'utilisation de `numpy` en traitement d'images :
...
**Attention :** il est important de noter que la convention pour les coordonnées change.

```python
img[l,c] # designe le pixel à la ligne l et à la colonne c
```

C'est donc **l'inverse** de la convention utilisée dans notre code C++.



<!-- 

TODO : quelques manips de base
### Travail à réaliser
...
La fonction `canny` de `scikit-image` implémente ce filtre, et une description est donnée ici :
https://scikit-image.org/docs/stable/api/skimage.feature.html#skimage.feature.canny

L'objectif de ce TP est d'implémenter le filtre de Canny puis de le comparer avec l'implémentation de `scikit-image`.
...
Pour calculer $`\arctan(\frac{y}{x})`$, utiliser la fonction `numpy.arctan2(y,x)`.
...
En Python, vous pourrez utiliser la classe `Queue` :
...
* Comparer le résultat avec la fonction `skimage.canny` de scikit-image en utilisant les mêmes paramètres (les paramètres ne sont pas tout à fait les mêmes, trouver un moyen de rendre les résultats comparables)
